...
namespace SVNStyleCop
{
    ...
    public class Program
    {
        ...
        {
            CommandLineArgument cla = new CommandLineArgument(args);
            if (!cla.Exists("repository") || !(cla.Exists("revision") || cla.Exists("transaction")))
            {
                PrintUsage();
                return -1;
            }
            ...
            SaveCommittedFilesToTemp(localSourceFiles, cla["repository"], cla["revision"], cla["transaction"]);

            string settingsFileOverride = string.Empty;
            if (cla.Exists("settings"))
            {
                settingsFileOverride = cla["settings"];
            }
        ...
        {
            Console.WriteLine("Utility to analyze source code files from SVN using StyleCop. Version {0}", Common.ApplicationVersion);
